...
| File | Role |
|---|---|
| `run-eval.sh` | Replays the suite against the Anthropic Messages API with the production request shape per surface (models, max_tokens, cache markers, LIBRARY hint blocks, vision routing + thinking-off). `KEEP_BODIES=1` keeps assembled request bodies. |
